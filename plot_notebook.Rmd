---
title: "Plots of Graphical Test for Discrete Uniformity and its Applications inGoodness of Fit Evaluation and Multiple Sample Comparison"
...
if (make_plots == TRUE) {
  ...
  qdunifordered <-
    function(q, N, i)
      optimize(function(t)
        abs(q - 1 + pbinom(i - 1, N, t)), c(0, 1))$minimum
  y <- rnorm(N_fig1, 0, 1)
  x <- replicate(N_fig1, rnorm(S_fig1, 0, 1))
  pits <- pnorm(y, 0, 1)
  u <- sapply(1:N_fig1, function(n)
    ecdf(x[, n])(y[n]))
  ...
  p_x <-
    ggplot(mapping = aes(x = (-90:90) / 30, y = dnorm((-90:90) / 30, 0, 1))) +
    geom_line(size = 1.8,
              colour = colors[3],
              alpha = 1) +
    theme(
  ...
  ecdf_pit <- ggplot(mapping = aes(x = pits, y = ecdf(pits)(pits))) +
    geom_step(size = 1.8, colour = colors[1]) +
    geom_segment(
      aes(
  ...
  u_z <- ggplot(mapping = aes(x = z, y = ecdf(u)(z))) +
    ...
    geom_segment(
      aes(
        x = z[bar_idx],
        y = qbinom(0.025, N_fig1, z[bar_idx]) / N_fig1 ,
        xend = z[bar_idx],
        yend = qbinom(0.925, N_fig1, z[bar_idx]) / N_fig1
      ),
      colour = colors[2],
      arrow = arrow(
        angle = 90,
...
```{r}
if (save == TRUE) {
  pdf(file="figures/normal_y_hist.pdf", width = fig_width, height = .6*fig_height)
  print(y_hist)
  dev.off()
  ggsave("figures/normal_x_hist1.pdf",plot=x_hist1, width = fig_width, height = .6*fig_height)
  ggsave("figures/normal_x_hist2.pdf",plot=x_hist2, width = fig_width, height = .6*fig_height)
  pdf(file="figures/normal_density.pdf", width = fig_width, height = .6*fig_height)
  print(p_x)
  dev.off()
